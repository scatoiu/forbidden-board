...
REASONING_BUDGET = {"off": 0, "high": 4096}
EFFORT_NOTE = {
...
for block in BLOCKS:
    ...
    for channel in CHANNELS:
        for effort in EFFORTS:
            for state in STATES:
                idx += 1
                sid = f"M{block}-{channel}-{effort}-{state}"
                seed = block * 100 + idx
